...
async def edit_task(task: EditTask, session: AsyncSession, user: UserInfo):
    ...
    if task_data.get('section_id'):
        ...
        check_user = check_user_project(project_id, user.id, session)
        if not check_user:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Вы не можете переместить задачу в проект, в котором Вас нет")
...
async def change_task_status(
    ...
    check_user = check_user_project(change_status.project_id, user.id, session)
    if check_user:
...
async def change_task_order(task_order: my_model.TaskOrder, session: AsyncSession, user: UserInfo):
    check_user = check_user_project(task_order.project_id, user.id, session)
    if check_user:
        new_order_list = list()
